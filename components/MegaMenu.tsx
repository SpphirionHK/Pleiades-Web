...
export function MegaMenu() {
  ...
  const activeFamily = productMenuFamilies[activeFamilyIndex] ?? productMenuFamilies[0];
  const activeTypes = activeFamily?.types ?? [];
  const safeTypeIndex = Math.min(activeTypeIndex, Math.max(activeTypes.length - 1, 0));
  const activeType = activeTypes[safeTypeIndex];
  ...
  return (
    <div className="absolute left-1/2 top-full z-40 hidden w-[1120px] -translate-x-1/2 border border-industrial-200 bg-white shadow-soft group-hover:block group-focus-within:block">
      <div className="grid min-h-[460px] grid-cols-[360px_330px_430px]">
        <div className="p-5">
